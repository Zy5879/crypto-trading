...
const BuySheet = forwardRef<BottomSheetModal, Props>(({ preset }, ref) => {
  ...
  const { data: prices, isFetching: fetchingPrice } = useGetMarketByIdsQuery(
    selected?.id ? { ids: [selected.id], vs: "usd" } : { ids: [] as string[] },
    { skip: !selected?.id }
  );
  const price = prices?.[0]?.current_price ?? 0;
  const name = selected?.name ?? prices?.[0]?.name ?? selected?.id ?? "Coin";
  const symbol = (selected?.symbol ?? prices?.[0]?.symbol ?? "").toUpperCase();
      ...
      <BottomSheetView style={s.wrap}>
        ...
        {showPicker ? (
          ...
          <>
            ...
            <Text style={s.meta}>
              Price:{" "}
              {fetchingPrice ? "…" : price ? `$${price.toLocaleString()}` : "—"}
            </Text>
            <BottomSheetTextInput
              style={s.input}
